...
const healthLevel= (points)=>points>=12?'bg-green-500':points>=6?'bg-yellow-500':`bg-red-500`;
...
const PlayerInfo = ({player,playerIcon,mt}) => {
  return (
    <div className={`${styles.flexCenter} ${mt?'mt-4':'mb-4'}`}>
      <img  data-for={`Player-${mt?'1':'2'}`} src={playerIcon} className='w-14 h-14 object-contain rounded-full'/>
  ...
  >
    {
      ...
      <ReactTooltip  id={`Health-${mt?'1':'2'}`} effect="solid" backgroundColor='#7f46f4'/>
      <ReactTooltip  id={`Mana-${mt?'1':'2'}`} effect="solid" backgroundColor='#7f46f4'/>
